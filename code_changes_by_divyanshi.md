# Walkthrough: Paper-Aligned Fixes to ESL Trainer

## Changes Made

### 1. [trainer.py](file:///home/batman/expts/esl/esl/trainer.py) — Two structural fixes
...
#### Fix 1: Simultaneous Belief Updates (Paper §3)

The inner `for (i, j) in E_t` loop was split into **three phases**:

| Phase | Lines | Purpose |
|:---|:---|:---|
| **Phase 1** (lines 369–416) | Collect interactions | Actions, payoffs, signals, and belief snapshots are gathered. **No beliefs modified.** |
| **Phase 2** (lines 418–461) | Apply updates | All belief updates applied simultaneously using start-of-round priors. Batch records built. |
| **Phase 3** (lines 463–489) | Prototype SGD | Round-based prototype update at round boundaries. |
...
#### Fix 2: Round-Based Prototype Updates (Paper Algorithm 1, lines 11–15)

- **Before:** `if interaction_n % Q == 0` — triggered **mid-round** inside the pair loop
- **After:** `if (t + 1) % Q == 0` — triggered **at round boundaries** after all pairs are processed

Q (`prototype_Q()`) is now interpreted as the number of **rounds** between prototype updates, matching the paper's `t mod M = 0`.

---

### 2. [test_esl_schedule.py](file:///home/batman/expts/esl/tests/test_esl_schedule.py) — Updated test
...
`test_q_triggers_multiple_sgd_within_one_round` → renamed to `test_q_triggers_round_based_prototype_updates`. Now tests Q=2 over 4 rounds, expecting 2 prototype steps at `t=1` and `t=3`.
...
- Integration tests (symmetric/asymmetric init, adaptation mode, recovery MCE)
- Schedule tests (Q-based updates, L2 regularization, degenerate protocol)
- Verification suite (gradient contracts, belief contracts, deterministic reproducibility)
- Convergence stopping tests
- Hand-trace belief verification tests

## Important Note

> [!IMPORTANT]
> **`prototype_update_every` now means rounds, not interactions.** If you had `prototype_update_every=15` meaning "every 15 interactions," you'll need to recompute the equivalent round count based on your average `L_t`. For example, with `L_t ∈ [5, 15]` (mean 10), 15 interactions ≈ 1.5 rounds, so you might set `prototype_update_every=2`.
